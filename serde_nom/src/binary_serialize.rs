...
// issue (#60551) describes the problem, must split into 2 traits (const-generic
// bugs) trait FromBytes {
...
macro_rules! derive_fb_primitive {
    ($name: ty) => {
        impl BinarySerializable for $name {
            fn deserialize<'a>(_: &'a [u8], input: &'a [u8]) -> IResult<&'a [u8], Self, VerboseError<&'a [u8]>>
            where
                Self: Sized,
            {
...
pub trait BinarySerializable {
    fn deserialize<'a>(all_input: &'a [u8], input: &'a [u8]) -> IResult<&'a [u8], Self, VerboseError<&'a [u8]>>
    where
        Self: Sized;
        ...
        Self: Sized,
    {
        let (rest, v) = Self::deserialize(input.0, input.1).map_err(|e| match e {
            nom::Err::Error(e) => {
                let errors = e.errors.into_iter().map(|(i, k)| ((input.0, i), k)).collect();
